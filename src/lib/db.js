...
export async function initializeUser(customWedding = null) {
  ...
  const wedding = customWedding ? {
    bride: customWedding.bride || TEMPLATE_WEDDING.bride,
    groom: customWedding.groom || TEMPLATE_WEDDING.groom,
    date: customWedding.date || TEMPLATE_WEDDING.date,
    venue: customWedding.venue || TEMPLATE_WEDDING.venue,
...
export async function initializeUser() {
  ...
  // Create wedding
  const year = new Date(TEMPLATE_WEDDING.date).getFullYear();
  await supabase.from('weddings').insert({ ...TEMPLATE_WEDDING, user_id: userId });
  ...
  // Also save to localStorage for immediate use
  saveState('wedding', TEMPLATE_WEDDING);
  saveState('guests', TEMPLATE_GUESTS.map((g, i) => ({ ...g, id: i + 1, group: g.group_name, inviteCode: makeInviteCode(g.name, year) })));
  saveState('budgetItems', TEMPLATE_BUDGET.map((b, i) => ({ ...b, id: i + 1, desc: b.description })));
  saveState('timeline', TEMPLATE_TIMELINE.map((e, i) => ({ ...e, id: i + 1, endTime: e.end_time, desc: e.description })));
  saveState('guestPageConfig', TEMPLATE_CONFIG);
